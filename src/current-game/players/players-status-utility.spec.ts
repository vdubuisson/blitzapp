...
import { PlayerStatus, PlayerStatusEnum } from '@/shared/types/player-status';
import {
...
describe('PlayersStatusUtility', () => {
  let spectator: SpectatorService<PlayersStatusUtility>;

  const createService = createServiceFactory({
    service: PlayersStatusUtility,
  });
  ...
  describe('addStatusToPlayer', () => {
    ...
    it('should add a new status to an empty statuses set', () => {
      const updated = spectator.service.addStatusToPlayer(
        basePlayer,
  ...
  describe('removeStatusFromPlayer', () => {
    ...
    it('should remove an existing status from the statuses set', () => {
      ...
      const updated = spectator.service.removeStatusFromPlayer(
        playerWithStatuses,
        PlayerStatusEnum.WOLF_TARGET,
      );
  ...
  describe('addStatusToPlayersById', () => {
    ...
    it('should add a status to the specified player and keep others unchanged', () => {
      ...
      const updated = spectator.service.addStatusToPlayersById(
        players,
        PlayerStatusEnum.PROTECTED,
        [0, 2],
      );
  ...
  describe('removeStatusFromPlayers', () => {
    ...
    it('should remove the status from specified players and keep others unchanged', () => {
      ...
      const updated = spectator.service.removeStatusFromPlayersById(
        players,
        PlayerStatusEnum.WOLF_TARGET,
        [0, 2],
      );
