...
export class StatusHandlersManager {
  private readonly injector = inject(Injector);

  private readonly currentPlayersState =
    inject(CurrentPlayersStore).state.asReadonly();

  private readonly statusHandlers = new Map<PlayerStatus, StatusHandler>();
   ...
   */
  getHandler(status: PlayerStatus): StatusHandler {
  ...
  createStatusHandler(status: PlayerStatus): void {
    if (this.statusHandlers.has(status)) {
      return;
    }
