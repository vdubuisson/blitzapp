...
export class RolesChoicePage implements OnInit {
  ...
  protected readonly roleCountForm: FormGroup<RoleForm> = this.formBuilder.group({
    loupGarou: [0, Validators.min(0)],
    villageois: [0, Validators.min(this.requiredVillageois())],
  });

  private readonly roleCountFormChanges = toSignal(this.roleCountForm.valueChanges);
